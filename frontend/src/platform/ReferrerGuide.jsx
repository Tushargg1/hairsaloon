...
// Renders text with **bold** segments highlighted for easy skimming.
function Highlight({ text }) {
  const parts = text.split(/(\*\*[^*]+\*\*)/g)
  return parts.map((part, i) =>
    part.startsWith('**') && part.endsWith('**')
      ? <strong key={i} className="text-on-surface font-semibold">{part.slice(2, -2)}</strong>
...
const SECTIONS = [
  {
    ...
    icon: 'rocket_launch',
    points: [
      'A **complete growth platform** for salons — **not just a website**. Each feature solves a real problem:',
      ...
      'Headline result: salons **retain 90%+ of customers** and can get **up to 2x more new leads**.',
  ...
  },
  {
    ...
    points: [
      ...
      'Use the WhatsApp scripts **in order** — message 1, 2, 3, then follow-ups A, B, C. **Never all at once.** Space them out; **stop after three unanswered**.',
    ],
  },
  {
    ...
    icon: 'quiz',
    points: [
      '**"How much does it cost?"** → "There is a **free demo** so you can try it first. I will share the exact plan once you have seen it work — most salons find it **pays for itself with a few extra bookings**." **Do not quote a number you are unsure of.**',
      '**"Do you have other clients I can check?"** → "Yes, we work with salons like yours. I can share **references** once you are seriously considering it — for now, the preview built for your salon is the fastest way to judge it."',
      '**"Is the AI actually AI, or canned replies?"** → "It **understands customer questions and books real appointments** — not fixed templates. The demo shows it replying live."',
      '**"I already use [competitor]."** → "Great that you are already online. This brings **booking, WhatsApp automation, AI chat and sales insights into one place** — many salons switch to **stop juggling tools**. Worth a 2-minute look?"',
      ...
      'If you do not know an answer, **never bluff**. Say "Let me confirm and get back to you" and **check with your Unitechverse manager**.',
    ],
  },
  {
    ...
    icon: 'sell',
    points: [
      'Lead with the **free demo** — no charge to look, and it removes hesitation.',
      '**Do NOT invent prices.** Use only the current plans and discount limits given by your **Unitechverse manager**.',
      'Asked for a number early? **Anchor on value first** ("pays for itself with a few extra bookings"), then share the exact plan **after the demo**.',
      'After the demo, the **Unitechverse team handles final pricing** and onboarding — your job is to get an **interested "yes"**.',
      '**[Fill in for your team: price, free-trial length, and what is negotiable.]**',
    ],
  },
  {
    ...
    icon: 'verified_user',
    points: [
      'Only message salons on **publicly listed business numbers**, for a **genuine business enquiry** — that is outreach, **not spam**.',
      '**Do not mass-blast.** Send personally, space out follow-ups, and **stop after three unanswered** messages.',
      'The salon\'s customer messaging runs on the **official WhatsApp Business API** with **customer opt-in** and **Meta-approved templates** — set up by the Unitechverse team, not by you.',
      '**Never promise bulk unsolicited WhatsApp messaging** — it violates Meta policy. Position it as **opt-in automation for their own customers**.',
    ],
  },
  {
    title: 'Keep in mind',
